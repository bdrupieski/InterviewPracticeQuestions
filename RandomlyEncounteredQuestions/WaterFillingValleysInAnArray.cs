...
namespace RandomlyEncounteredQuestions
{
    ...
    {
        private static int GetVolume(int[] arr)
        {
            int leftIndex = 0;
        ...
        {
            Assert.AreEqual(17, GetVolume(new[] { 2, 5, 1, 3, 1, 2, 1, 7, 7, 6 }));
            Assert.AreEqual(0, GetVolume(new[] { 1, 2, 1 }));

            Assert.AreEqual(1, GetVolume(new[] { 2, 1, 2 }));
            Assert.AreEqual(2, GetVolume(new[] { 5, 4, 4, 5 }));
            Assert.AreEqual(3, GetVolume(new[] { 1, 0, 0, 0, 1 }));

            Assert.AreEqual(2, GetVolume(new[] { 1, 0, 1, 0, 1 }));
            Assert.AreEqual(5, GetVolume(new[] { 2, 0, 1, 0, 2 }));
            Assert.AreEqual(4, GetVolume(new[] { 2, 0, 2, 0, 2 }));

            Assert.AreEqual(0, GetVolume(new[] { 6, 5, 4, 3, 2, 1 }));
            Assert.AreEqual(0, GetVolume(new[] { 1, 2, 3, 4, 5, 6 }));

            Assert.AreEqual(7, GetVolume(new[] { 7, 4, 3, 7, 6, 5 }));
            Assert.AreEqual(7, GetVolume(new[] { 5, 6, 7, 3, 4, 7 }));


            Assert.AreEqual(0, GetVolume(new[] { 1, 2, 3, 4, 5, 4, 3, 2, 1 }));
            Assert.AreEqual(0, GetVolume(new[] { 1, 2, 3, 4, 5, 4, 3, 2, 1 }));

            Assert.AreEqual(5, GetVolume(new[] { 3, 5, 2, 7, 8, 4, 6, 4 }));
            Assert.AreEqual(5, GetVolume(new[] { 4, 6, 4, 8, 7, 2, 5, 3 }));

            Assert.AreEqual(24, GetVolume(new[] { 4, 0, 3, 0, 2, 0, 5, 2, 1, 0, 4 }));
            Assert.AreEqual(24, GetVolume(new[] { 4, 0, 1, 2, 5, 0, 2, 0, 3, 0, 4 }));
        }
    }
}